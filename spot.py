    ...
    try:
        ...
        if data['total'] > 0 and data['total'] < 400:
            ...
            for result in data['results']:
                try:
                    if result['name'] not in name and result['detail_info']['tag'] == '旅游景点':
                        name.append(result['name'])
                    elif result['name'] not in name and len(result['detail_info']['tag'].split(';')) > 0:
                        if result['detail_info']['tag'].split(';')[0] == '旅游景点' and                         result['detail_info']['tag'].split(';')[1] != '其他' and result['detail_info']['tag'].split(';')[1] != '景点':
                            name.append(result['name'])
                except:
                    print(result)
            if page_numbers > 1:
...
def func_spot(city, branch_no, baidu_ak, radius, workbook):
